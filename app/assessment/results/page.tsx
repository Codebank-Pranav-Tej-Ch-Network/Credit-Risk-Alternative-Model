...
useEffect(() => {
    ...
    if (storedData) {
      ...
      // --- DYNAMIC LOGIC: Generate Profile ID ---
      const initials = (formData.fullName || "XX").split(' ').map(n => n[0]).join('');
      const timestamp = Date.now().toString().slice(-6);
      const dynamicId = `${initials}${timestamp}`;

      // --- DYNAMIC LOGIC: Generate Approval Reasons ---
      const generateReasons = () => {
        let reasons = [];
        const cibilScore = parseInt(formData.cibilScore, 10);
        const averageBalance = parseFloat(formData.averageBalance);

        if (apiResponse.prediction === 1) { // Approved
          if (cibilScore > 780) {
            reasons.push(`Excellent CIBIL score of ${cibilScore} demonstrates strong creditworthiness.`);
          } else {
            reasons.push("A strong credit history was a key positive factor in this decision.");
          }
          if (averageBalance > 50000) {
            reasons.push("Healthy average bank balance indicates excellent financial stability.");
          }
          if (apiResponse.probability_of_approval > 0.9) {
            reasons.push("Our model predicts a very low default risk based on your overall profile.");
          }
        } else { // Rejected
          if (cibilScore < 700) {
            reasons.push(`The CIBIL score of ${cibilScore} is below the required threshold for approval.`);
          }
          if (averageBalance < 10000) {
            reasons.push("Low average bank balance suggests potential financial strain.");
          }
          if (apiResponse.probability_of_approval < 0.5) {
            reasons.push("Our model predicts a high risk of default based on the provided data.");
          }
        }
        return reasons.length > 0 ? reasons : ["The decision was based on a comprehensive analysis of the provided data."];
      };

      // --- DYNAMIC LOGIC: Generate "Plausible" Feature Importance ---
      const generateFeatureImportance = () => {
        const cibil = parseInt(formData.cibilScore, 10);
        const balance = parseFloat(formData.averageBalance);
        const reach = parseInt(formData.socialMediaScore, 10) * 100;

        let cibilImpact = 0.4;
        if (cibil > 800 || cibil < 680) cibilImpact += 0.1;

        let balanceImpact = 0.2;
        if (balance > 100000 || balance < 5000) balanceImpact += 0.05;

        let reachImpact = 0.1;
        if (reach > 10000) reachImpact += 0.05;

        const totalImpact = cibilImpact + balanceImpact + reachImpact;

        return [
          { feature: "CIBIL Score", impact: parseFloat((cibilImpact/totalImpact).toFixed(2)) },
          { feature: "Bank Transaction Average", impact: parseFloat((balanceImpact/totalImpact).toFixed(2)) },
          { feature: "Social Media Reach", impact: parseFloat((reachImpact/totalImpact).toFixed(2)) },
        ];
      };

      // --- DYNAMIC LOGIC: Generate Risk Factor Status ---
      const defaultRiskScore = Math.round((1 - apiResponse.probability_of_approval) * 100);
      let riskStatus = "Medium";
      if (defaultRiskScore < 20) riskStatus = "Low";
      if (defaultRiskScore > 50) riskStatus = "High";

      const formattedResult = {
        decision: apiResponse.prediction === 1 ? "Approved" : "Rejected",
        confidence: parseFloat((apiResponse.probability_of_approval * 100).toFixed(1)),
        applicantName: formData.fullName || "N/A",
        loanAmount: parseFloat(formData.loanAmount) || 0,
        creditScore: parseInt(formData.cibilScore, 10) || 0,
        riskLevel: apiResponse.probability_of_approval > 0.7 ? "Low" : apiResponse.probability_of_approval > 0.4 ? "Medium" : "High",
        profileId: dynamicId,
        approvalReasons: generateReasons(),
        modelExplanation: {
            topFeatures: generateFeatureImportance(),
        },
        riskFactors: [
            { factor: "Default Risk", score: defaultRiskScore, weight: 50, status: riskStatus }
        ],
        dataSourceScores: [
            { source: "Credit Bureau", score: parseInt(formData.cibilScore, 10) / 10, reliability: 92 },
            { source: "Banking Data", score: Math.min(100, parseFloat(formData.averageBalance) / 1000), reliability: 85 },
            { source: "Social Media", score: parseInt(formData.socialMediaScore, 10), reliability: 60 },
        ],
      };
  ...
  // --- END NEW LOGIC ---
// Make sure this block is right after your new useEffect
